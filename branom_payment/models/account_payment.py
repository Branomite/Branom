...
class AccountPayment(models.Model):
    ...
    refund_invoice_ids = fields.Many2many(comodel_name='account.invoice', string='Credit Notes', compute='_compute_refund_invoice_ids', store=False)

    def _compute_refund_invoice_ids(self):
        for payment in self:
            payment.refund_invoice_ids = payment.invoice_ids.mapped('refund_invoice_ids')
    ...
    def _compute_payment_amount(self, invoices=None, currency=None, with_discount=True):
        ...
        invoice_datas = invoices.read_group(
            [('id', 'in', invoices.ids)],
            ['currency_id', 'type', 'residual_signed', 'residual_company_signed', 'payment_term_id'],
        ...
        for invoice_data in invoice_datas:
            ...
            amount_total = sign * invoice_data['residual_signed'] * (1.0 - discount)
            
            amount_total_company_signed = sign * invoice_data['residual_company_signed'] * (1.0 - discount)
